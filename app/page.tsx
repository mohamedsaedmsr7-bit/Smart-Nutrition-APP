...
// ========== قاعدة البيانات الشاملة (كل الأصناف من الجدول) ==========
const FOOD_DATABASE = [
  ...
  { id: 20, name: "Baked Sweet Potato - بطاطا حلوة مشوية", state: "Baked", type: "نشويات", carbs: 24, protein: 2, fat: 0.1, calories: 105 },
  ...
  { id: 28, name: "Raw Chicken Liver - كبدة دجاج نية", state: "Raw", type: "بروتين", carbs: 2, protein: 18, fat: 5, calories: 120 },
  { id: 29, name: "Raw Lean Beef - لحم بقري صافي ني", state: "Raw", type: "بروتين", carbs: 0, protein: 26, fat: 10, calories: 200 },
  { id: 30, name: "Cooked Lean Beef - لحم بقري صافي مطبوخ", state: "Cooked", type: "بروتين", carbs: 0, protein: 30, fat: 12, calories: 250 },
  { id: 31, name: "Cooked Fatty Beef - لحم بقري سمين مطبوخ", state: "Cooked", type: "بروتين", carbs: 0, protein: 25, fat: 20, calories: 300 },
  ...
  { id: 35, name: "Whole Eggs (Gm) - بيض كامل (جم)", state: "Raw", type: "بروتين", carbs: 1, protein: 13, fat: 11, calories: 155 },
  ...
  { id: 38, name: "1 Whole Egg - بيضة كاملة واحدة", state: "1 Egg", type: "بروتين", carbs: 0.3, protein: 6, fat: 5, calories: 65 },
  { id: 39, name: "Canned Tuna (Water) - تونة معلبة (ماء)", state: "Canned", type: "بروتين", carbs: 0, protein: 25, fat: 1, calories: 116 },
  { id: 40, name: "Canned Tuna (Oil) - تونة معلبة (زيت)", state: "Canned", type: "بروتين", carbs: 0, protein: 25, fat: 10, calories: 200 },
  ...
  { id: 45, name: "Plain Greek Yogurt - زبادي يوناني سادة", state: "Plain", type: "بروتين", carbs: 4, protein: 10, fat: 5, calories: 97 },
  ...
  { id: 53, name: "Ghee (Samna Baladi) - سمن بلدي", state: "Raw", type: "دهون", carbs: 0, protein: 0, fat: 100, calories: 900 },
  ...
  // 🍹 Fruit Juice (عصائر)
  { id: 87, name: "Orange Juice", state: "Fresh", type: "عصائر", carbs: 10, protein: 0.7, fat: 0.2, calories: 45 },
  { id: 88, name: "Mango Juice", state: "Fresh", type: "عصائر", carbs: 14, protein: 0.4, fat: 0.1, calories: 60 },
  { id: 89, name: "Apple Juice", state: "Fresh", type: "عصائر", carbs: 11, protein: 0.1, fat: 0.1, calories: 46 },
  ...
  { id: 118, name: "Raw Green Beans - فاصوليا خضراء نية", state: "Raw", type: "خضار", carbs: 7, protein: 2, fat: 0.1, calories: 31 },
  { id: 119, name: "Cooked Green Beans - فاصوليا خضراء مطبوخة", state: "Cooked", type: "خضار", carbs: 7, protein: 2, fat: 0.1, calories: 35 },
  ...
  { id: 134, name: "Ready Lentil Soup - شوربة عدس جاهزة", state: "Ready", type: "خضار", carbs: 14, protein: 5, fat: 2, calories: 90 },
  { id: 135, name: "Green Salad - سلطة خضراء", state: "Ready", type: "خضار", carbs: 4, protein: 1, fat: 0.2, calories: 20 },
  { id: 136, name: "Green Salad + Lemon - سلطة خضراء بالليمون", state: "Ready", type: "خضار", carbs: 4, protein: 1, fat: 0.2, calories: 22 },
...
export default function NutritionPlanner() {
  ...
  // تحديث الفئات لتشمل المكسرات والعصائر والمشروبات
  ...
  const calculateTotals = () => {
    return meals.reduce((acc, meal) => {
      ...
      return {
        cal: acc.cal + mealTotals.cal,
        pro: acc.pro + mealTotals.pro,
  ...
  const addItem = (mealId: number, food: any) => {
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-8 font-sans" dir="rtl">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Header */}
        <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 mb-8 flex justify-between items-center no-print shadow-2xl">
          <h1 className="text-2xl font-black text-blue-500 flex items-center gap-2">
            <Calculator size={30} /> برو نيوتريشن
          </h1>
          <button onClick={() => window.print()} className="bg-neutral-800 px-5 py-2 rounded-xl font-bold flex items-center gap-2 border border-neutral-700">
            <Printer size={18} /> PDF
          </button>
        </div>

        {/* Global Filter & Search */}
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 mb-8 sticky top-4 z-20 no-print shadow-xl">
          <div className="relative mb-4">
            <Search className="absolute right-3 top-3 text-neutral-500" size={20} />
            <input 
              type="text" 
              placeholder={`ابحث في ${FOOD_DATABASE.length} صنف...`} 
              className="w-full bg-black border border-neutral-700 rounded-xl py-3 pr-10 outline-none focus:border-blue-500 transition-all"
              value={searchTerm}
          ...
          <div className="flex flex-wrap gap-2">
            {foodTypes.map(t => (
              <button 
                ...
                className={`px-3 py-1.5 rounded-full text-[11px] font-black border transition-all ${activeFilter === t ? 'bg-blue-600 border-blue-600 text-white' : 'bg-neutral-800 border-neutral-700 text-neutral-400'}`}
              >
                {t}
              </button>
            ))}
          ...
          {meals.map((meal) => (
            <div key={meal.id} className="bg-neutral-900 border border-neutral-800 rounded-3xl overflow-hidden shadow-lg">
              <div className="p-5 flex items-center justify-between bg-neutral-800/40 border-b border-neutral-800">
                <h2 className="text-xl font-black">{meal.name}</h2>
                <span className="text-sm font-bold bg-black/40 px-3 py-1 rounded-full border border-neutral-800">
              ...
              <div className="p-6">
                  ...
                  {meal.items.map((item: any) => (
                    <div key={item.instanceId} className="flex items-center justify-between bg-black/40 p-3 rounded-2xl border border-neutral-800/50">
                      ...
                      <div className="flex items-center gap-3">
                        <input 
                          type="number" 
                          className="w-16 bg-neutral-800 border border-neutral-700 rounded-lg p-1 text-center text-sm font-black no-print" 
                          value={item.grams} 
                          onChange={(e) => updateGrams(meal.id, item.instanceId, parseFloat(e.target.value) || 0)} 
                        />
                        <span className="text-xs font-bold w-12">{item.grams}ج</span>
                        <button onClick={() => removeItem(meal.id, item.instanceId)} className="text-neutral-700 hover:text-red-500 no-print transition-colors"><X size={18}/></button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {/* Selection Panel */}
                <div className="no-print border-t border-neutral-800 pt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                    {filteredDatabase.map(f => (
                      <button 
                        key={f.id} 
                        onClick={() => addItem(meal.id, f)} 
                        className="flex justify-between items-center p-3 bg-neutral-800/20 border border-neutral-800/60 rounded-xl hover:bg-blue-600/10 hover:border-blue-500/40 text-right group transition-all"
                      >
                        <div className="flex flex-col items-start">
                           <span className="text-xs font-bold group-hover:text-blue-400">{f.name}</span>
                           <span className="text-[9px] text-neutral-500">{f.calories} kcal / 100g</span>
                        </div>
                        <Plus size={16} className="text-neutral-600 group-hover:text-blue-400" />
                      </button>
                    ))}
                  </div>
                </div>
